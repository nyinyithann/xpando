...
describe('except()', () => {
  ...
  test('should exclude items passed as arguments', () => {
    expect(new Vec(1, 2, 3).except(true)).toEqual(new Vec(1, 2, 3));
    expect(new Vec(1, 2, 3, 4, 5).except(true, 2, 3, 4)).toEqual(new Vec(1, 5));
    expect(new Vec([1], [2], [3], [4], [5]).except(true, [2], [3], [4])).toEqual(new Vec([1], [5]));
    expect(new Vec({ n: 1 }, { n: 1 }, { n: 2 }, { n: 2 }, { n: 5 })
      .except(true, { n: 1 }, { n: 1 }, { n: 2 })).toEqual(new Vec({ n: 5 }));
  ...
  test('invocation via call/apply/bind should be fine', () => {
    ...
    const except = Vec.prototype.except;
    expect(except.call(vec, true, 2, 3, 4)).toEqual(new Vec(1, 5));
    expect(except.apply(vec, [true, 2, 3, 4])).toEqual(new Vec(1, 5));
    expect(except.bind(vec)(true, 2, 3, 4)).toEqual(new Vec(1, 5));
  });
});
